...
* [**Completion:**](/sdk/ai-tasks/completion) LLM inference for text generation and chat via [`llama.cpp`](https://github.com/ggml-org/llama.cpp).
* [**Text embeddings:**](/sdk/ai-tasks/text-embeddings) vector embedding generation for semantic search, clustering, and retrieval, via `llama.cpp`.
* [**Translation:**](/sdk/ai-tasks/translation) text-to-text neural machine translation (NMT), via [Marian](https://marian-nmt.github.io) and [Bergamot](https://browser.mt).
* [**Transcription:**](/sdk/ai-tasks/transcription) automatic speech recognition (ASR) for speech-to-text via [`whisper.cpp`](https://github.com/ggml-org/whisper.cpp).
* [**Text-to-Speech:**](/sdk/ai-tasks/text-to-speech) speech synthesis for text-to-speech (TTS) via [ONNX Runtime](https://onnxruntime.ai).
* [**OCR:**](/sdk/ai-tasks/ocr) optical character recognition (OCR) for extracting text from images via ONNX runtime.
* [**Multimodal:**](/sdk/ai-tasks/multimodal) LLM inference over text, images, and other media within a single conversation context.
* [**RAG:**](/sdk/ai-tasks/rag) out-of-the-box retrieval-augmented generation workflow.

### P2P capabilities

* [**Delegated inference:**](/sdk/p2p/delegated-inference) delegate inference to peers via the [Holepunch stack](https://holepunch.to), enabling resource sharing.
* **Fetch models:** download AI models from peers via the distributed model registry.
* [**Blind relays:**](/sdk/p2p/blind-relays) connect peers across NATs/firewalls by routing traffic through relay nodes.

### Utilities

* [**Logging:**](/sdk/utilities/logging) visibility into what's happening  during loading, inference, and other operations.
* [**Download Lifecycle:**](/sdk/utilities/download-lifecycle) pause and resume model downloads.
* [**Sharded models:**](/sdk/utilities/sharded-models) download a model that is sharded into multiple parts.
...
<Cards className="grid-cols-1">
  <Card href="/sdk/utilities/plugin-system" title="Built-in and custom plugins">
    Enable and disable built-in AI capabilities, and add new ones via custom plugins.
  </Card>

  <Card href="/sdk/utilities/write-custom-plugin" title="Write a custom plugin">
